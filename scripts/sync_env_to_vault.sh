...
log_info "Syncing Vault CA certificate to Kubernetes ConfigMap 'vault-ca' across namespaces..."
for ns in mail-and-media-shop-dev mam-dev mam-qa mam-live; do
  kubectl create namespace "$ns" --dry-run=client -o yaml | kubectl apply -f - || true
  $SSH_CMD "sudo cat /opt/vault/tls/tls.crt" | kubectl create configmap vault-ca --from-file=ca.crt=/dev/stdin -n "$ns" --dry-run=client -o yaml | kubectl apply -f - || true
done
